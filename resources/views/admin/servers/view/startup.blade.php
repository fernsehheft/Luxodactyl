...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        $('#pEggId').select2({placeholder: 'Select a Nest Egg'}).on('change', function () {
            ...
            var parentChain = _.get(Luxodactyl.nests, $("#pNestId").val());
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (objectChain.id === parseInt(Luxodactyl.server.egg_id) && Luxodactyl.server.image == opt.value) {
                    opt.selected = true
                }
            ...
            if (objectChain.id === parseInt(Luxodactyl.server.egg_id)) {
                if ($('#pDockerImage').val() != Luxodactyl.server.image) {
                    $('#pDockerImageCustom').val(Luxodactyl.server.image);
                }
            }
            ...
            $.each(_.get(objectChain, 'variables', []), function (i, item) {
                var setValue = _.get(Luxodactyl.server_variables, item.env_variable, item.default_value);
        ...
        $('#pNestId').select2({placeholder: 'Select a Nest'}).on('change', function () {
            ...
            if (_.isObject(_.get(Luxodactyl.nests, $(this).val() + '.eggs.' + Luxodactyl.server.egg_id))) {
                $('#pEggId').val(Luxodactyl.server.egg_id);
            }
