    ...
    return (
        <PageContentBlock title={action === 'edit' ? `Editing ${filename}` : `New File`} className='p-0! h-full'>
            ...
            {['.pyroignore', '.pteroignore'].includes(filename) ? (
                <div className={`mb-4 p-4 border-l-4 bg-mocha-400 rounded-md border-brand`}>
                    <p className={`text-neutral-300 text-sm`}>
                        You&apos;re editing a{' '}
                        <code className={`font-mono bg-mocha-500 rounded-md py-px px-1`}>.pteroignore</code> file. Any
                        files or directories listed in here will be excluded from backups. Wildcards are supported by
                        using an asterisk (<code className={`font-mono bg-mocha-500 rounded-md py-px px-1`}>*</code>
                        ). You can negate a prior rule by prepending an exclamation point (
                        <code className={`font-mono bg-mocha-500 rounded-md py-px px-1`}>!</code>
                        ).
                    </p>
                </div>
            ) : null}
